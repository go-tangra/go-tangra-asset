...
/**
 * Calls the asset API through the gateway. Non-2xx responses reject with
 * ApiError carrying the server's closed-vocabulary `reason`. This client is
 * JSON-only; uploads go through upload().
 */
...
/** Uploads a file as multipart/form-data (photo/document routes). */
export async function upload<T = unknown>(path: string, file: File, fields: Record<string, string> = {}): Promise<T> {
  ...
  let res: Response
  try {
    res = await fetch(url, { method: 'POST', headers: { Accept: 'application/json', [CSRF_HEADER]: csrfToken() }, credentials: 'same-origin', body: form })
  } catch {
    throw new ApiError(0, 'network')
  }
  const data: unknown = await res.json().catch(() => ({}))
...
/** URL of a binary route (photo, document download) for <img>/<a> use. */
