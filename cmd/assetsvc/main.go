// Command assetsvc runs the asset service. `assetsvc -config <path>` starts
// the service (applying migrations); `assetsvc bootstrap -config <path>`
...
import (
	...
	"github.com/go-freya/freya/services/asset/ui"
)

func main() {
	if len(os.Args) > 1 && os.Args[1] == "bootstrap" {
		if err := bootstrap(os.Args[2:]); err != nil {
			fmt.Fprintln(os.Stderr, "assetsvc:", err)
			os.Exit(1)
		}
		return
	}
...
func loadConfig(args []string) (config.Config, error) {
	fs := flag.NewFlagSet("assetsvc", flag.ContinueOnError)
	...
	for _, w := range cfg.Warnings() {
		fmt.Fprintln(os.Stderr, "assetsvc: warning:", w)
	}
