...
func TestRoutesValidationBranches(t *testing.T) {
	...
	routes, err = Routes(docWith(map[string]any{
		PermissionExtension: "assets:read",
	...
	if routes[0].Permission != "assets:read" || routes[0].MaxBodyBytes != 2048 || routes[0].Timeout.Seconds() != 30 {
	...
	if _, err := Routes(docWith(map[string]any{PermissionExtension: "assets:read", BodyLimitExtension: float64(0)})); err == nil || !strings.Contains(err.Error(), "bad body limit") {
		t.Fatalf("bad body limit: %v", err)
	}
	// bad body limit (wrong type).
	if _, err := Routes(docWith(map[string]any{PermissionExtension: "assets:read", BodyLimitExtension: "big"})); err == nil {
		t.Fatal("bad body limit type should error")
	}
	// bad timeout (out of range).
	if _, err := Routes(docWith(map[string]any{PermissionExtension: "assets:read", TimeoutExtension: float64(9999)})); err == nil || !strings.Contains(err.Error(), "bad timeout") {
