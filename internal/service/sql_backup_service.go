...
// the schema-agnostic sqldump engine over asset's own tables (from ent
// migrate.Tables). Replaces the legacy per-field asset.service.v1.BackupService,
...
func NewSqlBackupService(ctx *bootstrap.Context) *SqlBackupService {
	...
	return &SqlBackupService{
		log:    ctx.NewLoggerHelper("asset/service/sql-backup"),
		engine: sqldump.New(dsn, sqldump.Options{Module: "asset", Tables: tables}),
	}
}
...
func (s *SqlBackupService) ImportBackup(stream commonV1.BackupService_ImportBackupServer) error {
	...
	if opts == nil {
		return errors.New("asset: first ImportBackup message must carry options")
	}
	...
	if err != nil {
		...
		return stream.SendAndClose(&commonV1.ImportBackupResponse{Success: false, Module: "asset", Warnings: []string{err.Error()}})
	}
	out := &commonV1.ImportBackupResponse{Success: true, Module: "asset", Warnings: res.Warnings}
