// Package authz is the asset (ITAM) access model. Authorization is coarse (no
// Zanzibar): a caller is scoped to a tenant and carries roles the gateway
// resolved for it. This package answers "may this caller act within this
// tenant?", "is this caller a tenant admin?" and "is this caller a platform
// admin?" — the last gates privileged, cross-tenant maintenance operations that
// a mere tenant admin must not reach.
package authz
...
// trusted system scope used by scheduler/maintenance paths.
const (
	ActorUser    = "user"
...
// "admin" and "system" roles all confer full control; the system actor scope is
// likewise treated as admin so trusted scheduler paths pass admin gates.
func (s Subjects) IsAdmin() bool {
	if s.ActorKind == ActorSystem {
		return true
	}
	return s.hasRole(RoleAdmin) || s.hasRole(RoleOwner) || s.hasRole(RoleSystem)
}

// IsPlatformAdmin reports whether the caller may perform privileged, cross-tenant
// maintenance operations. The dedicated "platform-admin" role grants it; the
// tenant "admin"/"owner"/"system" roles and the system actor scope also satisfy
// it so operators and trusted worker paths are not locked out.
func (s Subjects) IsPlatformAdmin() bool {
	if s.ActorKind == ActorSystem {
		return true
	}
	return s.hasRole(RolePlatformAdmin) || s.hasRole(RoleAdmin) || s.hasRole(RoleOwner) || s.hasRole(RoleSystem)
}
...
// guards cross-tenant maintenance operations.
