// Package audit records every asset (ITAM) operation in a closed vocabulary with
// a detail guard that keeps supplier/location contact fields, e-mail/phone and
// object-store credentials out of the log. Events are buffered and written to the
// audit hypertable by a background goroutine so recording never blocks the caller.
package audit
...
// Event types.
const (
	AssetCreated EventType = "asset_created"
	AssetUpdated EventType = "asset_updated"
	AssetDeleted EventType = "asset_deleted"
	...
	SupplierUpdated EventType = "supplier_updated"
	SupplierDeleted EventType = "supplier_deleted"
	...
	ConsumableCreated EventType = "consumable_created"
	ConsumableUpdated EventType = "consumable_updated"
	ConsumableDeleted EventType = "consumable_deleted"

	LicenseCreated EventType = "license_created"
	LicenseUpdated EventType = "license_updated"
	LicenseDeleted EventType = "license_deleted"

	InsuranceCreated EventType = "insurance_created"
	InsuranceUpdated EventType = "insurance_updated"
	InsuranceDeleted EventType = "insurance_deleted"

	AssetAssigned   EventType = "asset_assigned"
	AssetUnassigned EventType = "asset_unassigned"

	PhotoUploaded EventType = "photo_uploaded"
	PhotoDeleted  EventType = "photo_deleted"

	DocumentUploaded EventType = "document_uploaded"
	DocumentDeleted  EventType = "document_deleted"

	PolicyAssetAdded   EventType = "policy_asset_added"
	PolicyAssetRemoved EventType = "policy_asset_removed"
...
const (
	...
	SubjectConsumable  = "consumable"
	SubjectLicense     = "license"
	SubjectInsurance   = "insurance"
	SubjectPolicyAsset = "policy_asset"
	SubjectSync        = "sync"
...
func init() {
	for _, t := range []EventType{
		...
		ConsumableCreated, ConsumableUpdated, ConsumableDeleted,
		LicenseCreated, LicenseUpdated, LicenseDeleted,
		InsuranceCreated, InsuranceUpdated, InsuranceDeleted,
		AssetAssigned, AssetUnassigned,
...
type Event struct {
	...
	ActorID     string
	SubjectKind string // asset | assignment | document | ... | system
	SubjectID   string
	Target      string // external id/tag being acted on (active ops)
...
func Validate(e Event) error {
	...
	case SubjectAsset, SubjectAssignment, SubjectDocument, SubjectCategory,
		SubjectSupplier, SubjectLocation, SubjectConsumable, SubjectLicense,
		SubjectInsurance, SubjectPolicyAsset, SubjectSync, SubjectBackup,
		SubjectSystem:
	default:
...
// forbidden detail-key substrings (case-insensitive): supplier/location contact
// fields, e-mail/phone and object-store credentials never belong in a detail.
var forbidden = []string{"secret", "credential", "contact", "email", "phone", "telephone", "password"}
...
// secret|credential|contact|email|phone|telephone|password (at any depth)
// dropped and string values truncated to 256 characters.
