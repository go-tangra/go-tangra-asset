...
func (_q *LocationQuery) QueryAssets() *AssetQuery {
...
func (_q *LocationQuery) QueryConsumables() *ConsumableQuery {
...
func (_q *LocationQuery) WithAssets(opts ...func(*AssetQuery)) *LocationQuery {
	query := (&AssetClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *LocationQuery) WithConsumables(opts ...func(*ConsumableQuery)) *LocationQuery {
	query := (&ConsumableClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *LocationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Location, error) {
	...
	if query := _q.withAssets; query != nil {
		if err := _q.loadAssets(ctx, query, nodes,
			func(n *Location) { n.Edges.Assets = []*Asset{} },
			func(n *Location, e *Asset) { n.Edges.Assets = append(n.Edges.Assets, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withConsumables; query != nil {
		if err := _q.loadConsumables(ctx, query, nodes,
			func(n *Location) { n.Edges.Consumables = []*Consumable{} },
			func(n *Location, e *Consumable) { n.Edges.Consumables = append(n.Edges.Consumables, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *LocationQuery) loadAssets(ctx context.Context, query *AssetQuery, nodes []*Location, init func(*Location), assign func(*Location, *Asset)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Location)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(asset.FieldLocationID)
	}
	query.Where(predicate.Asset(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(location.AssetsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *LocationQuery) loadConsumables(ctx context.Context, query *ConsumableQuery, nodes []*Location, init func(*Location), assign func(*Location, *Consumable)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Location)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(consumable.FieldLocationID)
	}
	query.Where(predicate.Consumable(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(location.ConsumablesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
