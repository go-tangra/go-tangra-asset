...
func (_q *CategoryQuery) QueryAssets() *AssetQuery {
...
func (_q *CategoryQuery) QueryConsumables() *ConsumableQuery {
...
func (_q *CategoryQuery) WithAssets(opts ...func(*AssetQuery)) *CategoryQuery {
	query := (&AssetClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *CategoryQuery) WithConsumables(opts ...func(*ConsumableQuery)) *CategoryQuery {
	query := (&ConsumableClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *CategoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Category, error) {
	...
	if query := _q.withAssets; query != nil {
		if err := _q.loadAssets(ctx, query, nodes,
			func(n *Category) { n.Edges.Assets = []*Asset{} },
			func(n *Category, e *Asset) { n.Edges.Assets = append(n.Edges.Assets, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withConsumables; query != nil {
		if err := _q.loadConsumables(ctx, query, nodes,
			func(n *Category) { n.Edges.Consumables = []*Consumable{} },
			func(n *Category, e *Consumable) { n.Edges.Consumables = append(n.Edges.Consumables, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *CategoryQuery) loadAssets(ctx context.Context, query *AssetQuery, nodes []*Category, init func(*Category), assign func(*Category, *Asset)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Category)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(asset.FieldCategoryID)
	}
	query.Where(predicate.Asset(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(category.AssetsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *CategoryQuery) loadConsumables(ctx context.Context, query *ConsumableQuery, nodes []*Category, init func(*Category), assign func(*Category, *Consumable)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Category)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(consumable.FieldCategoryID)
	}
	query.Where(predicate.Consumable(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(category.ConsumablesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
